...
class Monitor(object):
    ...
    def __init__(self,
        ...
        map_coord_x, map_coord_y = np.meshgrid(np.arange(resolution[1]),
    ...
    def remap(self):
        ...
        for j in np.arange(resolution[1]):
            ...
            for i in np.arange(resolution[0]):
    ...
    def plot_map(self):
        ...
        mapcorX, mapcorY = np.meshgrid(np.arange(resolution[1]), np.arange(resolution[0]))
        ...
        levels1 = np.arange(int(np.floor(self.lin_coord_x.min() / 10) * 10),
        ...
        levels2 = np.arange(int(np.floor(self.lin_coord_y.min() / 10) * 10),
        ...
        levels3 = np.arange(int(np.floor(self.deg_coord_x.min() / 10) * 10),
        ...
        levels4 = np.arange(int(np.floor(self.deg_coord_y.min() / 10) * 10),
    ...
    def warp_images(self, imgs, center_coor, deg_per_pixel=0.1, is_luminance_correction=True):
        ...
        alt_axis = alt_start - np.anp.arange(imgs_raw.shape[1]) * deg_per_pixel_alt
        azi_start = center_coor[1] - (imgs_raw.shape[2] / 2) * deg_per_pixel_azi
        azi_axis = np.anp.arange(imgs_raw.shape[2]) * deg_per_pixel_azi + azi_start
        ...
        for ii in np.arange(self.deg_coord_x.shape[0]):
            for jj in np.arange(self.deg_coord_x.shape[1]):
        ...
        if is_luminance_correction:
            for frame_ind in np.arange(imgs_wrapped.shape[0]):
        ...
        # crop image
        alt_np.arange = np.logical_and(np.anp.arange(imgs_raw.shape[1]) >= y_min,
                                   np.anp.arange(imgs_raw.shape[1]) <= y_max)
        azi_np.arange = np.logical_and(np.anp.arange(imgs_raw.shape[2]) >= x_min,
                                   np.anp.arange(imgs_raw.shape[2]) <= x_max)

        # print imgs_raw.shape
        # print imgs_raw.shape
        # print alt_np.arange.shape
        # print azi_np.arange.shape
        # print np.sum(alt_np.arange)
        # print np.sum(azi_np.arange)

        imgs_dewrapped = imgs_raw[:, alt_np.arange, :]
        imgs_dewrapped = imgs_dewrapped[:, :, azi_np.arange]

        # get degree coordinats of dewrapped images
        deg_coord_alt_ax_dewrapped = alt_axis[alt_np.arange]
        deg_coord_azi_ax_dewrapped = azi_axis[azi_np.arange]
